...
void Diagnostics::startDiagnostics(){
    ...
    sendDiagnosticsMessage(s.engineFound, "Engine sensor found!", "Enige sensor not found!");
    ...
    sendDiagnosticsMessage(!isnan(value[0]), "Receiced valid values from engine sensor!", "Received invalid values from engine sensor!");
    sendDiagnosticsMessage(!isnan(value[1]), "Receiced valid values from battery sensor!", "Received invalid values from battery sensor!");
    sendDiagnosticsMessage(!isnan(temp_sensor.getChipTemperature()), "Receiced valid values from chip sensor!", "Received invalid values from chip sensor!");
    
    Serial.println(">>>Checking Electrical Measurements<<<");
    Wire.begin();
    sendDiagnosticsMessage(electrical_measurement.init(&Wire, &Serial), "INA219 successfully initialized!" , "INA219 could not be initialized!");
    ...
    sendDiagnosticsMessage(!isnan(electrical_measurement.getVoltage()), "Received valid values for voltage!", "Received invalid values for current!");
